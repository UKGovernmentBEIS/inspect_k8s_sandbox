...
Selecting a ServiceAccount with `serviceAccountName` does not expose its Kubernetes API
token unless `automountServiceAccountToken` is also set to `true`. This keeps Kubernetes
API credentials separate from provider-specific workload identity tokens such as IRSA.

If a sandbox needs Kubernetes API access:

- Give it a dedicated ServiceAccount with only the required RBAC permissions.
