...
* Set `automountServiceAccountToken: false` on each sandbox Pod unless its code
  intentionally requires Kubernetes API access.
...
Only enable service-account token automount for a dedicated, least-privileged
ServiceAccount. See [Kubernetes workload
security](../security/kubernetes-workloads.md#service-accounts) for guidance.
