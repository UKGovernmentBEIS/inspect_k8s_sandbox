...
| allowDomainsPorts | list | Empty list (only 80/443 are reachable on allowedDomains) | Extra ports opened to the resolved IPs of allowDomains, as a list of {port, protocol, domain}. protocol defaults to ANY (TCP+UDP); domain (optional) scopes the port to one allowDomains entry, otherwise it applies to all of them. These are pinned by IP only with no SNI/Host check, so only open ports you trust on those IPs. 80 and 443 may only be listed with protocol UDP (e.g. QUIC); TCP/ANY there is rejected as it would disable the identity check above. |
