...
| allowDomainsPorts | list | Empty list (only 80/443 are reachable) | Extra ports opened to the resolved IPs of allowDomains, as a list of {port, protocol} (protocol defaults to ANY = TCP+UDP). These are pinned by IP only with no SNI/Host check, so only open ports you trust on those IPs. 80 and 443 may only be listed with protocol UDP (e.g. QUIC); TCP/ANY there is rejected as it would disable the identity check above. |
