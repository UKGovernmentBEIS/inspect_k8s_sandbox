...
async def _setup_slow_fifo(
    ...
    if mode == "read":
        ...
        cleanup_cmd = f"sleep {cleanup_delay}; touch {stop_file}"
    else:
        ...
        cleanup_cmd = (
            f"sleep {cleanup_delay}; touch {stop_file}; sleep 2; rm -f {fifo_path}"
        )
...
async def test_read_file_transient_fault(
    ...
    try:
        ...
        print(f"  PASS: read_file succeeded in {elapsed:.1f}s ({len(contents)} bytes)")
        return True
    except Exception as e:
        elapsed = time.time() - t0
        ...
        try:
            await sandbox.exec(
                [
                    "bash",
                    "-c",
                    f"touch {fifo_path}.stop; sleep 1; "
                    f"rm -f {fifo_path} {fifo_path}.stop",
                ]
            )
        except Exception:
            pass
...
async def test_write_file_transient_fault(
    ...
    try:
        ...
        result = await sandbox.exec(["bash", "-c", f"wc -c < {fifo_path}"])
        print(
        ...
        elapsed = time.time() - t0
        print(f"  FAIL: write_file raised {type(e).__name__} after {elapsed:.1f}s: {e}")
        return False
    finally:
        if not fault_task.done():
            fault_task.cancel()
