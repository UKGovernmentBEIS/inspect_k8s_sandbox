...
@pytest.fixture
def namespace() -> str:
    return get_current_context_namespace()
...
def test_without_cleanup(model: Model, task: Task, namespace: str) -> None:
    release = "no-clean"
    ...
    asyncio.run(uninstall(release, namespace, quiet=False))


def test_cli_cleanup_all_gets_user_confirmation(
    model: Model, task: Task, namespace: str
) -> None:
    release = "no-clean"
    with patch(
        "k8s_sandbox._helm.Release._generate_release_name",
